...
function capabilityText(audience: CustomAudienceView) {
  return (Object.keys(capabilityLabels) as Array<keyof CustomAudienceView["capabilities"]>)
    .map((capability) => `${capabilityLabels[capability]}: ${audience.capabilities[capability] === "available" ? "disponível" : audience.capabilities[capability] === "unavailable" ? "indisponível" : "não confirmada"}`)
    .join("; ");
}
...
export default function AudiencesPage() {
  ...
  if (!accountId) return <main className="p-6"><Card><CardContent className="p-6 text-sm text-muted-foreground">Nenhuma conta de anúncios acessível para este cliente.</CardContent></Card></main>;
  ...
  return <main className="mx-auto max-w-6xl space-y-6 p-6">
    ...
    {library ? <CustomerListImport accountId={accountId!} userId={userId} audiences={library.audiences} onChanged={() => setCursors((current) => [...current])} /> : null}
    {library ? <><InstagramAudienceEditor accountId={accountId!} userId={userId} onSaved={() => setCursors((current) => [...current])} /><WebsiteAudienceEditor accountId={accountId!} userId={userId} onSaved={() => setCursors((current) => [...current])} /><LookalikeAudienceCreator accountId={accountId!} userId={userId} audiences={library.audiences} onSaved={() => setCursors((current) => [...current])} /><Card><CardContent className="p-0"><ul className="divide-y">{library.audiences.map((audience) => <li key={audience.id} className="grid gap-3 p-5 text-sm md:grid-cols-5"><div><p className="font-medium">{audience.name ?? "Sem nome"}</p><p className="font-mono text-xs text-muted-foreground">{audience.id}</p><p className="text-xs text-muted-foreground">{audience.subtype ?? "Tipo não informado"}</p><div className="mt-2 flex flex-wrap gap-2">{audience.capabilities.editMetadata === "available" ? <AudienceMetadataEditor audience={audience} accountId={accountId!} userId={userId} onSaved={() => setCursors((current) => [...current])} /> : null}<AudienceDeletionControl audience={audience} accountId={accountId!} userId={userId} onDeleted={() => setCursors((current) => [...current])} /></div><div className="mt-2"><InstagramAudienceEditor accountId={accountId!} userId={userId} audience={audience} onSaved={() => setCursors((current) => [...current])} /><WebsiteAudienceEditor accountId={accountId!} userId={userId} audience={audience} onSaved={() => setCursors((current) => [...current])} /></div></div><div>{ruleText(audience)}{audience.retentionDays !== undefined ? <p className="text-xs text-muted-foreground">Período: {audience.retentionDays} dias</p> : null}</div><div>Processamento: {statusText(audience.operationStatus)}</div><div>Entrega: {statusText(audience.deliveryStatus)}{audience.availability ? <p className="text-xs text-muted-foreground">Incluir: {availabilityText(audience.availability.include)} · Excluir: {availabilityText(audience.availability.exclude)} · Lookalike: {availabilityText(audience.availability.lookalikeSource)}</p> : null}</div><div>{estimateText(audience)}{importResultText(audience) ? <p className={audience.availability?.lookalikeSource === "blocked" ? "text-xs text-destructive" : "text-xs text-muted-foreground"}>{importResultText(audience)}</p> : null}<p className="text-xs text-muted-foreground">{capabilityText(audience)}</p>{audience.originAudienceId ? <p className="font-mono text-xs text-muted-foreground">Origem: {audience.originAudienceId}</p> : null}</div></li>)}{library.audiences.length === 0 ? <li className="p-10 text-center text-sm text-muted-foreground">Nenhum público acessível nesta página.</li> : null}</ul></CardContent></Card><div className="flex items-center justify-between"><Button variant="outline" disabled={!cursors.length} onClick={() => setCursors((current) => current.slice(0, -1))}><ChevronLeft />Anterior</Button><span className="text-xs text-muted-foreground">Página {cursors.length + 1}</span><Button variant="outline" disabled={!library.hasNextPage || !library.nextCursor} onClick={() => library.nextCursor && setCursors((current) => [...current, library.nextCursor!])}>Próxima<ChevronRight /></Button></div><Card><CardContent className="space-y-1 p-4 text-xs text-muted-foreground">{library.limitations.map((item) => <p key={item}>{item}</p>)}</CardContent></Card></> : null}
  </main>;
}
