...
export default function AudiencesPage() {
  ...
  return <main className="mx-auto max-w-6xl space-y-6 p-6">
    ...
    {library ? <><Card><CardContent className="p-0"><ul className="divide-y">{library.audiences.map((audience) => <li key={audience.id} className="grid gap-3 p-5 text-sm md:grid-cols-5"><div><p className="font-medium">{audience.name ?? "Sem nome"}</p><p className="font-mono text-xs text-muted-foreground">{audience.id}</p><p className="text-xs text-muted-foreground">{audience.subtype ?? "Tipo não informado"}</p></div><div>{audience.ruleSummary === "external" ? "Regra externa" : "Sem regra representável"}{audience.retentionDays !== undefined ? <p className="text-xs text-muted-foreground">Período: {audience.retentionDays} dias</p> : null}</div><div>Processamento: {statusText(audience.operationStatus)}</div><div>Disponibilidade: {statusText(audience.deliveryStatus)}</div><div>{estimateText(audience)}<p className="text-xs text-muted-foreground">Consulta: disponível</p><p className="text-xs text-muted-foreground">Incluir, excluir, editar metadados/regra, membros, exclusão e origem de lookalike: não confirmados</p>{audience.originAudienceId ? <p className="font-mono text-xs text-muted-foreground">Origem: {audience.originAudienceId}</p> : null}</div></li>)}{library.audiences.length === 0 ? <li className="p-10 text-center text-sm text-muted-foreground">Nenhum público acessível nesta página.</li> : null}</ul></CardContent></Card><div className="flex items-center justify-between"><Button variant="outline" disabled={!cursors.length} onClick={() => setCursors((current) => current.slice(0, -1))}><ChevronLeft />Anterior</Button><span className="text-xs text-muted-foreground">Página {cursors.length + 1}</span><Button variant="outline" disabled={!library.hasNextPage || !library.nextCursor} onClick={() => library.nextCursor && setCursors((current) => [...current, library.nextCursor!])}>Próxima<ChevronRight /></Button></div><Card><CardContent className="space-y-1 p-4 text-xs text-muted-foreground">{library.limitations.map((item) => <p key={item}>{item}</p>)}</CardContent></Card></> : null}
  </main>;
}
