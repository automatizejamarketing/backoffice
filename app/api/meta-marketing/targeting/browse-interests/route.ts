...
export async function GET(
  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          message: "An ad account is required to browse interests",
    ...
    const response = await metaApiCall<MetaInterestSearchResponse>({
      method: "GET",
      path: "search",
      params: buildInterestBrowseParams({ locale }),
    ...
    console.error("Error browsing interests:", error);
    return NextResponse.json(
      {
        ...
        message:
          error instanceof Error
            ? error.message
            : "An unexpected error occurred while browsing interests",
