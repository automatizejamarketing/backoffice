...
export async function GET(
  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          message: "An ad account is required to suggest interests",
    ...
    let names: string[] = [];
    try {
      names = JSON.parse(namesParam) as string[];
      if (!Array.isArray(names)) names = [];
    } catch {
      names = namesParam ? [namesParam] : [];
    }
    ...
    const response = await metaApiCall<MetaInterestSearchResponse>({
      method: "GET",
      path: "search",
      params: buildInterestSuggestionParams({ names, locale }),
    ...
    return NextResponse.json(
      {
        ...
        message:
          error instanceof Error
            ? error.message
            : "An unexpected error occurred while suggesting interests",
