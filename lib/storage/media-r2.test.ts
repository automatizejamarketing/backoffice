...
process.env.MEDIA_PUBLIC_BASE_URL = "https://www.automatizemarketing.com";
...
describe("media-r2 URLs", () => {
  it("gera URL pública a partir da chave, com e sem o prefixo media/", () => {
    assert.equal(
      ...
      "https://www.automatizemarketing.com/api/media/posts/abc-123.png",
    );
    assert.equal(
  ...
  it("aceita URL relativa da mesma rota", () => {
    assert.equal(
  ...
  it("recusa path traversal e lixo", () => {
    assert.equal(mediaUrlToKey("/api/media/../../etc/passwd"), null);
    assert.equal(mediaUrlToKey("/api/media/"), null);
...
describe("media-r2 configuração", () => {
  ...
  it("reconhece as credenciais de product assets como fallback", () => {
    assert.equal(isMediaR2Configured(valid), true);
  });
  ...
  it("trata valores mascarados/curtos como não configurado", () => {
    ...
    assert.equal(
      isMediaR2Configured({ ...valid, PRODUCT_ASSETS_R2_BUCKET: "" } as NodeJS.ProcessEnv),
      false,
    );
  });
