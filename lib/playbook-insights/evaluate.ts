...
export function evaluatePlaybookInsights(args: {
  ...
  const cpaAlert = args.cpaAlertThreshold ?? PLAYBOOK_DEFAULT_CPA_ALERT;
  ...
  for (const campaign of args.campaigns) {
    ...
    if (
      status === "ACTIVE" &&
      campaign.spend >= PLAYBOOK_MIN_SPEND &&
      roas !== null &&
      roas <= PLAYBOOK_ROAS_TRIGGER
    ) {
      candidates.push({
        ...
        evidence: `Campanha "${campaign.name}" com ROAS ${formatRoas(roas)} (≤ ${PLAYBOOK_ROAS_TRIGGER}) e gasto R$ ${campaign.spend.toFixed(2)} nos últimos 30d.`,
        recommendation:
    ...
    if (
      ...
      candidates.push({
        ...
        evidence: `Campanha "${campaign.name}" com ROAS ${formatRoas(roas)} (≥ ${PLAYBOOK_ROAS_VALIDATED}) e gasto R$ ${campaign.spend.toFixed(2)}.`,
        recommendation:
          status === "ACTIVE"
    ...
    if (
      ...
      campaign.spend >= PLAYBOOK_MIN_SPEND_STALLED
    ) {
      const pausedDays = daysSince(now, campaign.updatedTime);
      if (pausedDays !== null && pausedDays >= PLAYBOOK_STALLED_DAYS) {
        candidates.push({
