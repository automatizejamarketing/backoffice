...
export type AudienceCommandRecord = { commandId: string; status: AudienceCommandStatus; result?: unknown; acquired?: boolean };
  ...
  return {
    async claim(input) {
      return db.transaction(async (tx) => {
        ...
        if (existing) return { ...toRecord(existing), acquired: false };
    ...
    async get(commandId) { const [row] = await db.select().from(metaAudienceCommand).where(eq(metaAudienceCommand.commandId, commandId)).limit(1); return row ? toRecord(row) : undefined; },
