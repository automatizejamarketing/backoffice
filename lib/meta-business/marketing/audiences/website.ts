...
export const WEBSITE_PERIOD_EVIDENCE: Record<WebsiteAudienceCriterion, WebsitePeriodEvidence> = Object.fromEntries(([
  "visitors", "url", "event",
] as WebsiteAudienceCriterion[]).map((criterion) => [criterion, { sourceId: undefined, criterion, initialDays: null, editable: "unknown", metaMinimumDays: null, metaMaximumDays: null, localValidationMaximumDays: null, unit: "days", historicalFill: "unknown", observedAt: "2026-09-10", context: "V02: a confirmação v25 pelo Gerenciador não está disponível em execução headless; a divergência entre a UI e os exemplos da API não foi fechada por inferência.", source: "Documentação oficial Meta v25 de públicos de site; sem observação autenticada da interface do Gerenciador." } satisfies WebsitePeriodEvidence])) as Record<WebsiteAudienceCriterion, WebsitePeriodEvidence>;
...
export function websitePeriodSelectionsEqual(left: WebsiteAudienceSelection | null | undefined, right: WebsiteAudienceSelection): boolean { return Boolean(left && left.pixelId.trim() === right.pixelId.trim() && left.criterion === right.criterion && left.retentionDays === right.retentionDays); }
